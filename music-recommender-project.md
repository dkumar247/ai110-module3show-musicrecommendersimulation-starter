...
## Course Info
- **Course:** AI110 | Foundations of AI Engineering — Spring 2026 (a Section 1E | Wednesdays 6PM - 8PM PT)
- **Week:** 7
- **Estimated Time:** ~4 hours
- **Deadline:** Project 3 is due by **Monday, April 13th at 2:59AM EDT**
...
1. Explain the transformation of data into predictions, distinguishing between input features, user preferences, and ranking algorithms.
2. Design and implement a weighted-score recommender in Python that uses attributes like genre, mood, and energy to calculate song relevance.
3. Identify and document algorithmic bias, using AI to brainstorm how simple content-based systems might create "filter bubbles" or over-favor certain genres.
4. Communicate technical reasoning through a structured Model Card that details intended use, data limitations, and future improvements.
...
- Take a **screenshot** of your terminal output for each profile's recommendations (song titles, scores, and reasons) and include it in your `README.md`.
...
- Introduce **5 or more** complex attributes that are not currently present in the baseline data, such as **Song Popularity (0-100)**, **Release Decade**, or **Detailed Mood Tags** (e.g., "nostalgic," "aggressive," "euphoric").
...
| File | Purpose |
|------|---------|
| `data/songs.csv` | Song catalog with attributes: `genre`, `mood`, `energy`, `tempo_bpm` |
| `src/recommender.py` | Core logic: `load_songs()`, `score_song(user_prefs, song)`, `recommend_songs(user_prefs, songs, k)` |
| `src/main.py` | CLI entry point, user profiles, runs recommendations |
| `README.md` | "How The System Works" section, screenshots, personal reflection |
| `model_card.md` | Model Card with 8 sections (see Phase 5) |
| `reflection.md` | Profile comparisons and evaluation comments |
...
The `score_song` function must return:
- A **numeric score**
- A **list of "reasons"** (e.g., `"genre match (+2.0)"`)

The `recommend_songs` function must:
- Loop through all songs and score each one
